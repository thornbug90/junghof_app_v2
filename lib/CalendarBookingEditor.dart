...
import 'package:junghof_app_v2/firebase.dart';

/// Render the widget of appointment editor calendar
class CalendarBookingEditor extends StatefulWidget {
  /// creates the appointment editor

  StateSetter setter;
  CalendarBookingEditor(this.setter);

  @override
  State<CalendarBookingEditor> createState() => _CalendarBookingEditorState();
}
...
class _CalendarBookingEditorState extends State<CalendarBookingEditor> {
  _CalendarBookingEditorState();
  ...
  Future<dynamic> getData() async {
    await _getAppointmentDetails().then((List<Appointment> snapshot) async {
      setState(() {
...
//// the given width agenda view will render the web UI.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: calendarController.view == CalendarView.month &&
          ...
          : Container(color: cardThemeColor, child: calendar),
    );
  }
  ...
  void _onCalendarTapped(CalendarTapDetails calendarTapDetails) {
    ...
    if (!isWebFullView && calendarController.view == CalendarView.month) {
  ...
  Future<List<Appointment>> _getAppointmentDetails() async {
    final List collection = await fetchDB();
    ...
    for (int i = 0; i < collection.length; i++) {
      var dataMap = collection[i];
      appointmentCollection.add(Appointment(
          startTime: DateTime.fromMillisecondsSinceEpoch(
              dataMap['startTime'].millisecondsSinceEpoch),
          endTime: DateTime.fromMillisecondsSinceEpoch(
              dataMap['endTime'].millisecondsSinceEpoch),
          color: Color(dataMap['color']),
          startTimeZone: dataMap['starTimeZone'],
          endTimeZone: dataMap['endTimeZone'],
          notes: dataMap['notes'],
          subject: dataMap['subject'],
          isAllDay: dataMap['isAllDay'],
          resourceIds: dataMap['resourceIds'],
          id: dataMap['id'],
          recurrenceRule: dataMap['recurrenceRule']));
    }
...
class _ResourcePickerState extends State<_ResourcePicker> {
  final Color backgroundColor = const Color.fromRGBO(0, 116, 227, 1);
...
class _AppointmentEditorWebState extends State<AppointmentEditorWeb> {
  ...
  void _monthWeekIcon() {
    setState(() {
      ...
      _monthIconColor = Colors.black54;
      _lastDayIconColor = Colors.black54;
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          onPressed:
                                                                            ...
                                                                            final DateTime? pickedDate = await showDatePicker(
                                                                                ...
                                                                                builder: (BuildContext context, Widget? child) {
                                                                                  return Theme(
                                                                                    data: ThemeData(brightness: Brightness.light, colorScheme: _getColorScheme(true), primaryColor: backgroundColor),
                                                                                    child: child!,
                                                                                  );
                                                                                });
                                                                            if (pickedDate ==
                                ...
                                child: TextField(
                                  ...
                                  onChanged: (String value) {
                                    ...
                                    _recurrenceProperties!.dayOfMonth =
                                        _dayOfMonth;
                                    _weekIconColor = Colors.black54;
                                    _monthIconColor = backgroundColor;
                                    _monthDayRadio = Icons.radio_button_checked;
                                    _weekDayRadio =
...
ColorScheme _getColorScheme(bool isDatePicker) {
  ...
  return ColorScheme.light(
    ...
    surface: isDatePicker ? backgroundColor : Colors.white,
  );
}
  ...
  Widget build(BuildContext context) {
    ...
    return Theme(
        ...
        child: Scaffold(
                    ...
                    onPressed: () async {
                      if (widget.selectedAppointment != null) {
                        if (widget.selectedAppointment!.appointmentType !=
                          ...
                          Appointment newAppintment = Appointment(
                            startTime: _startDate,
                            endTime: _endDate,
                            color: widget.colorCollection[_selectedColorIndex],
                            startTimeZone: _selectedTimeZoneIndex == 0
                          ...
                          bool isOk = await isUpdateDb(
                              widget.selectedAppointment, newAppintment);
                          if (isOk) {
                            updateDB(widget.selectedAppointment, newAppintment);
                            appointment.add(newAppintment);
                        ...
                        //insert firestore
                        bool isOk = await isAddDB(addItem);
                        if (isOk) {
                          insertDB(addItem);
                          appointment.add(addItem);
                          widget.events.appointments!.add(appointment[0]);
                          widget.events.notifyListeners(
                              CalendarDataSourceAction.add, appointment);
                          Navigator.pop(context);
            ...
            floatingActionButton: isWebFullView
                ? null
                : widget.selectedAppointment == null
                    ? const Text('')
                    : FloatingActionButton(
                        onPressed: () {
                          if (widget.selectedAppointment != null) {
                            deleteDB(widget.selectedAppointment);
                            if (widget.selectedAppointment!.appointmentType ==
                                AppointmentType.normal) {
                              widget.events.appointments!.removeAt(widget
                                  .events.appointments!
